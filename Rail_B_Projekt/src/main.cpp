...
RailCounter rail_B_input = RailCounter();
...
void setup() {
  ...
  //andon_line_G = AndonLine(LOCATION, &hal.inputs[0], &hal.outputs[0], &hal.outputs[1], &hal.outputs[2]);
  rail_B_input = RailCounter(LOCATION, 8, &hal.inputs[1], &hal.inputs[0], &hal.inputs[3], &hal.inputs[2]);
}
